...
foreach ( $rowarray[$x] as $review ) 
{
	...
	if($currentform[0]->display_num>0){
		...
		//if we have readmore pop up turned on then add it here.
		//wprevpro_btn_show_rdpop
		$rdmorepopclassstart = "";
		$rdmorepopclassend = "";
		$media ="";
		if(isset($template_misc_array['readmpop']) && $template_misc_array['readmpop']=="yes"){
			$rdmorepopclassstart = "<span class='wprevpro_btn_show_rdpop'>";
			$rdmorepopclassend = "</span>";
			//media
			$media = $templateclass->wprevpro_get_media($review,$template_misc_array);
		}
		
		$tempreviewernamehtml = '<span class="t10_revname wprev_preview_tcolor1_T'.$currentform[0]->style.'">'.$tempreviewername.'</span> '.__('left us a ', 'wp-review-slider-pro').$rdmorepopclassstart.$review->rating.' star review'.$rdmorepopclassend;
		if($review->rating<1){
			//change 
			if($review->recommendation_type=='positive' || $review->recommendation_type=='negative'){
				$tempreviewernamehtml = '<span class="t10_revname wprev_preview_tcolor1_T'.$currentform[0]->style.'">'.$tempreviewername.'</span> '.__('left us a ', 'wp-review-slider-pro').$rdmorepopclassstart.$review->recommendation_type.' recommendation'.$rdmorepopclassend;
			}
		}
