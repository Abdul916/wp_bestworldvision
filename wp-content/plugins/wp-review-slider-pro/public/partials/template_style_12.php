...
for ($x = 0; $x < count($rowarray); $x++) {
	...
	if(	$currentform[0]->template_type=="widget"){
			$iswidget=true;
		?>
		<div class="wprevpro_t12_outer_div_widget w3_wprs-row-padding-small wprevprodiv">
		<?php
		} else {
			$iswidget=false;
		?>
		<div class="wprevpro_t12_outer_div w3_wprs-row wprevprodiv">
		<?php
	}
	}
	//loop 
	foreach ( $rowarray[$x] as $review ) 
	{
		...
		//star number --------------------------------------
		$starfile_burl_logo_array = $templateclass->wprevpro_get_star_logo_burl($review,$imgs_url,$currentform[0],"t12",$template_misc_array);
		$starfile = $starfile_burl_logo_array['starfile'];
		$logo =  "<div class='wprevt12_icon_div'>".$starfile_burl_logo_array['logo']."</div>";
		...
		$companyhtml = $templateclass->wprevpro_get_companyhtml($review,$template_misc_array,"t12");
		...
		$tempuserpic ='';
		if($userpic==""){
			$userpichtml = '<div class="wpproslider_t12_avatar_div"></div>';
		} else {
			$altname = htmlspecialchars(stripslashes(strip_tags($review->reviewer_name)));
			$userpichtml = '<div class="wpproslider_t12_avatar_div">'.$profilelink['start'].'<img loading="lazy" src="'.$userpic.'" alt="'.$altname.' Avatar" class="wpproslider_t12_IMG_2 wprevpro_avatarimg" />'.$profilelink['end'].'</div>';
			if(isset($template_misc_array['avataropt'])){
				if($template_misc_array['avataropt']=='hide'){
					...
					$userpichtml = '<div class="wpproslider_t12_avatar_div">'.$profilelink['start'].'<img loading="lazy" src="'.$tempuserpic.'" alt="'.$altname.' Avatar" class="wpproslider_t12_IMG_2 wprevpro_avatarimg"/>'.$profilelink['end'].'</div>';
		...
		$tempmasonrydiv ='wpproslider_t12_DIV_1 w3_wprs-col l'.$perrow;
		...
		if(	$currentform[0]->display_masonry=="yes" && $currentform[0]->createslider != "sli"){
			$tempmasonrydiv = $masonryclass_item.' wprevpro_t12_DIV_1';
		}
	...
	<div class="<?php echo $tempmasonrydiv; ?> outerrevdiv">
		<div class="indrevdiv wpproslider_t12_DIV_1a<?php if($iswidget){echo "_widget";} ?>  wprev_preview_bg1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?> wprev_preview_bradius_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
			...
			<div class="indrevtxt wpproslider_t12_P_5 wprev_preview_tcolor2_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $rdmorescroll[0]; ?>
				<?php echo $title; ?><?php echo stripslashes($reviewtext); ?><?php echo $rdmorescroll[1]; ?><?php echo $ownerresponse; ?>
			</div>
			<?php echo $userpichtml; ?>
			<div class="wpproslider_t12_H3_3 wprev_preview_tcolor1_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>">
				<?php echo $tempreviewername; ?><?php echo $verifiedstarhtmlarray[1]; ?><?php echo $companyhtml;?>
			</div>
			<div>
			<span class="wpproslider_t12_date wprev_preview_tcolor3_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"> <span class="wprev_showdate_T<?php echo $currentform[0]->style; ?><?php if($iswidget){echo "_widget";} ?>"><?php echo $datestring; ?></span></span>
			</div>
			<?php echo $logo; ?>
			<?php echo $media; ?>
