...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    /** @var \AmeliaStripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \AmeliaStripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \AmeliaStripe\Util\RequestOptions::parse([
     ...
     *
     * @return \AmeliaStripe\StripeObject the object returned by Stripe's API
     */
    public function request($method, $path, $params, $opts)
    {
        $opts = $this->defaultOpts->merge($opts, true);
        $baseUrl = $opts->apiBase ?: $this->getApiBase();
        $requestor = new \AmeliaStripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        if (!($obj instanceof \AmeliaStripe\Collection)) {
            ...
            $msg = "Expected to receive `AmeliaStripe\\Collection` object from Stripe API. Instead received `{$received_class}`.";

            throw new \AmeliaStripe\Exception\UnexpectedValueException($msg);
        }
    ...
    public function requestSearchResult($method, $path, $params, $opts)
    {
        ...
        if (!($obj instanceof \AmeliaStripe\SearchResult)) {
            $received_class = \get_class($obj);
            $msg = "Expected to receive `AmeliaStripe\\SearchResult` object from Stripe API. Instead received `{$received_class}`.";
     ...
     *
     * @throws \AmeliaStripe\Exception\AuthenticationException
     *
     * @return string
     */
    ...
    private function validateConfig($config)
    {
        ...
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \AmeliaStripe\Exception\InvalidArgumentException('files_base must be a string');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \AmeliaStripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
}
